...
func TestBootstrapBalances(t *testing.T) {
	...
	t.Run("Set balance successfully", func(t *testing.T) {
		...
		assert.NoError(t, ioutil.WriteFile(bootstrapBalancesFile, file, utils.DefaultFilePermissions))
	...
	t.Run("Invalid file contents", func(t *testing.T) {
		assert.NoError(t, ioutil.WriteFile(bootstrapBalancesFile, []byte("bad file"), utils.DefaultFilePermissions))
